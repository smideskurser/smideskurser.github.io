---
layout: faq
permalink: /faq/
title: Vanliga frågor 
---

# Vad är en smideskurs?

Våra smideskurser är praktiska utbildningar där du lär dig grunderna i hur man formar metall med eld och olika verktyg. Du får prova på klassiskt järnsmide, skapa egna föremål och förstå hur metall reagerar på värme och kraft.

# Hur mycket erfarenhet behöver jag för att gå en smideskurs?

Ingen alls. Våra kurser är byggda för nybörjare, och du får en trygg introduktion till både verktyg och teknik. Inga förkunskaper krävs – bara nyfikenhet och viljan att arbeta med händerna.

# Vad är en svetskurs?

På våra svetskurser får du lära dig hur du korrekt använder MMA-svets, samt MIG/MAG-svets, säkerhetsutrustning, och svetsteknik. Du kommer att få svetsa ihop både praktiska och snygga saker som du självklart får ta med dig hem. 

# Hur börjar man med metallarbete?

Det enklaste sättet att börja är att gå [en introduktionskurs i smide](/smide/). Där får du lära dig de viktigaste teknikerna, använda ässjan och prova att forma järn. Efter kursen kan du fortsätta hemma eller gå en fortsättningskurs.


# Vad får jag göra på kurserna?

På våra smideskurser får lära dig hur man formar järn med hammare, städ, och andra verktyg, använder ässjan. Andra kurser lär ut olika tekniker som svetsning och gjutning. Många deltagare tillverkar något eget, till exempel en krok, ljusstake eller liten kniv.

# Är smide farligt?

Allt arbete med metall kräver respekt för värme och verktyg, men under kursen arbetar du alltid under handledning. Vi går igenom säkerheten noggrant, tillhandahåller skyddsutrustning och visar rätt arbetssätt för att undvika skador.

# Vad ska jag ha med mig?

Bekväma oömma kläder i bomull, stängda skor och eventuellt ett förkläde om du har ett. All annan utrustning – verktyg, material och skydd – finns på plats.

# Hur många deltagare är det per kurs?

Vi håller grupperna små, ofta 4–6 personer, så att alla får gott om tid och personlig handledning.

# Kan jag boka en privat eller företagskurs?

Ja, vi erbjuder skräddarsydda tillfällen för företag, grupper eller privatpersoner. [Kontakta oss]({{ site.url }}/#contact) så hittar vi ett upplägg som passar er.

# Hur långa är kurserna?

Från en till två dagar. Längden står alltid angiven vid respektive kurs.

# Var hålls kurserna?

Kurserna hålls i vår fullt utrustade verkstad vid Gunnestorpsvägen 25 i Mjöhult på vackra Kullahalvön strax norr om Helsingborg. Varje deltagare får sin egen arbetsplats. Vi erbjuder även företagskurser och gruppbokningar.

# Vad kostar det?

Priset varierar beroende på kursens längd och innehåll. Våra kurser kostar mellan 2 500 – 4 800 kr, beroende på längd och nivå, vilket inkluderar material och handledning. Fullständig information finns [i kursernas beskrivning.]({{ site.url }}/#portfolio)

# Får jag ta med det jag tillverkar hem?

Självklart. Allt du tillverkar under kursen är ditt. Det brukar vara något att vara stolt över – och ett minne av att du faktiskt har skapat något av metall med egna händer.

# Är det verkligen något för mig?

Om du gillar att skapa något konkret, vill prova något utanför vardagen och uppskattar doften av järn och eld – då är svaret ja. Det ser kraftfullt ut, men smide handlar mer om teknik än styrka. Du lär dig hur järnet “beter sig” och hur du formar det effektivt. Många som aldrig hållit i en hammare tidigare blir förvånade över hur snabbt de kommer in i det.